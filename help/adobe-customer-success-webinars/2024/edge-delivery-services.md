...
* 其目標是提供卓越的體驗，透過快速的開發週期和100%的燈塔分數來推動參與和轉換。 &#x200B;

### 製作選項

* **檔案式撰寫**&#x200B;使用熟悉的工具（如Microsoft Word或Google Docs）來建立內容，無需經過大量訓練即可快速建立內容。 &#x200B;
* **通用編輯器**&#x200B;提供與傳統ATM網站類似的WYSIWYG介面，允許建立更詳細且更視覺化的內容。 &#x200B;

### 架構

* EDS整合至Amazon Cloud Service架構。 &#x200B;
* 它支援無伺服器實作，且不需要傳統作者或發佈者執行個體即可運作。 &#x200B;
* 可以實作兩個快取層級：在客戶基礎架構層級和EDS層級。 &#x200B;

### 內容管理

* 檔案式製作需要GitHub帳戶、Google Drive或Microsoft SharePoint、Sidekick外掛程式和程式碼同步工具。 &#x200B;
* 使用IAM編寫的EDS需要GitHub帳戶、IAM as a Cloud Service授權和程式碼同步工具。

### 開發和部署

* 使用EDS建立網站的程式很快，通常只需要不到一天的時間。 &#x200B;
* 本機開發可使用aem up命令來建立網站的本機版本。
* 變更可以提交至功能分支，以供在合併至主要分支之前進行測試。 &#x200B;

### 自訂性與擴充性

* 您可以使用簡單的CSS和JavaScript來建立自訂元件。 &#x200B;
* 此架構可提供協力廠商整合和自訂編寫來源。
...
* 提供完整的檔案，以引導使用者完成設定和自訂程式。 &#x200B;
* 如有任何未解決的問題，建議使用者聯絡Adobe支援。 &#x200B;
