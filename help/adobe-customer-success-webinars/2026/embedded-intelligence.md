---
title: 內嵌式智慧轉換
description: 探索Adobe Experience Cloud如何引進輔助智慧，以強化決策並加速insight對動作的週期。
solution: Experience Platform
feature: AI Assistant, Personalization
topic: Artificial Intelligence
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 3008
last-substantial-update: 2026-01-14T00:00:00Z
jira: KT-20075
exl-id: 80806f06-8d5a-480c-8b18-6efc622a4c77
source-git-commit: 460acb3fd1e9b29075cefa07e8d6947d2a61a314
workflow-type: tm+mt
source-wordcount: '75'
ht-degree: 0%

---

# 我們之間的代理程式：內嵌式智慧轉換

探索人工智慧如何在Adobe Experience Cloud中從概念轉換為功能。 瞭解內嵌智慧的演變，並探索可強化分析、內容和客戶參與度的早期應用程式。 瞭解組織如何準備資料、團隊和治理，以負責任且有效地採用這些功能。

>[!VIDEO](https://video.tv.adobe.com/v/3479031/?learn=on&enablevpops)
