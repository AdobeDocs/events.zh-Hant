...
請求使用者存取報告ProofHQ時，這是否也會授予他們左側區段的存取權（即工作流程、連絡人、帳戶設定等）？
...
如果其他使用者拒絕或核准變更，是否有方法可以觸發新決策。 即校訂部門看到某樣東西，專案經理必須做出新的決定……即使他們已檢視並核准該專案。 （嘗試不讓專案經理等校訂部門進行檢閱）？
...
如果您在Workfront的搜尋列右側有一個核取方塊圖示，表示您擁有整合的Workfront和校訂帳戶。 按一下該核取方塊即可進入Workfront Proof，主畫面為控制面板。 您可以使用左側面板中的檢視選項來建置報表。
