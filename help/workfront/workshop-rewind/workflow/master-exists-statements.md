---
title: 您的夢想Workfront報告已存在！ 掌握EXISTS陳述式的兩種方法
description: 技術訣竅與實用應用程式，向我們展示如何使用EXISTS陳述式來建置更聰明的報表
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 3755
last-substantial-update: 2025-12-02T00:00:00Z
jira: KT-19787
source-git-commit: 2a5336b19f61bb0e1c48311b36c76c4a0967c410
workflow-type: tm+mt
source-wordcount: '165'
ht-degree: 0%

---


# 您的夢想Workfront報告已存在！ 掌握EXISTS陳述式的兩種方法

準備好將您的Workfront報表提升到新的層級了嗎？  看看Skye Hansen和Nathan Johnson如何提供兩種觀點來建立EXISTS陳述式，讓您實現報表夢想。

在這場研討會中，他們分享了技術訣竅和應用程式，向我們展示如何使用EXISTS陳述式來建置更聰明的報表。

>[!VIDEO](https://video.tv.adobe.com/v/3477158/?learn=on&enablevpops)

您是否錯過即時工作階段？ 沒關係！ 請檢閱Skye &amp; Nathan的[簡報投影片](https://experienceleaguecommunities.adobe.com/t5/workfront-discussions/event-follow-up-your-dream-workfront-report-exists-two/td-p/783550?profile.language=zh-Hant)，並觀看隨選錄製。

您是否有想法的分享或活動中的後續問題？ 歡迎在[Experience League社群貼文](https://workfront-experience.s3.us-west-2.amazonaws.com/Training/Guides/Customer+Success+at+Scale/10.28.25+EXISTS+Class.pdf)的註解中捨棄這些註解！

每月都會新增事件，因此請務必檢視[Experience League事件頁面](https://experienceleague.adobe.com/zh-hant/events)以瞭解最新工作階段。

