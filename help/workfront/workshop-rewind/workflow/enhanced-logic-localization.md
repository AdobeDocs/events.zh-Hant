---
title: 擷取更聰明、可擴充至全球 — 自訂Forms與Workfront本地化
description: 探索全新進階企業營運系列的更多功能，包括專為更聰明的資料管理和全球擴充而設計的增強自訂Forms和自訂本地化。
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 2705
last-substantial-update: 2026-06-10T00:00:00Z
jira: KT-21577
source-git-commit: 3ba1c8a76e6929f47b3a72683e7bde0ba45a01b2
workflow-type: tm+mt
source-wordcount: '173'
ht-degree: 0%

---


# 擷取更聰明、可擴充至全球 — 自訂Forms與Workfront本地化

在第二期「進階企業營運(AEO)功能事件系列」中，我們的Adobe產品經理Khachatur Sedrakyan和Kristine Tumanyan探索了四種新的進階邏輯型別、顯示、可編輯性、條件式格式和預設值，以實現更動態、自動化和資料精確的擷取體驗，並引入新的本地化增強功能，允許團隊為使用者輸入的標籤新增自訂翻譯。

>[!VIDEO](https://video.tv.adobe.com/v/3492142/?learn=on&enablevpops)

正在尋找更多嗎？ 訂閱我們的[進階企業營運活動系列Experience League貼文](https://experienceleaguecommunities.adobe.com/adobe-workfront-23/event-series-advanced-enterprise-operations-capabilities-250546?profile.language=zh-Hant)，在新增工作階段、註冊連結、錄製作業和資源時，自動接收通知！

每月都會新增事件，因此請務必檢視[Experience League事件頁面](https://experienceleague.adobe.com/zh-hant/events?lang=en)以瞭解最新工作階段。
