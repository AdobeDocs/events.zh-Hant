---
title: 快速追蹤效率 — 使用Workfront Fusion匯入資料
description: 此課程涵蓋3種主要的CSV匯入/匯出案例 — 從CSV檔案匯入任務、建立下拉式清單的自訂表單欄位值，以及將專案資料匯出為CSV檔案。
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 3633
last-substantial-update: 2025-12-03T00:00:00Z
jira: KT-19778
source-git-commit: 2a5336b19f61bb0e1c48311b36c76c4a0967c410
workflow-type: tm+mt
source-wordcount: '134'
ht-degree: 0%

---


# 快速通道效率：使用Workfront Fusion匯入資料

此研討會與Adobe Professional Services的Brian Hight合作主辦，涵蓋3個主要的CSV匯入/匯出案例：從CSV檔案匯入任務、建立下拉式清單的自訂表單欄位值，以及將專案資料匯出為CSV檔案。

>[!VIDEO](https://video.tv.adobe.com/v/3477127/?learn=on&enablevpops)

檢閱[投影片組](https://workfront-experience.s3.us-west-2.amazonaws.com/Training/Guides/Customer+Success+at+Scale/Slide+Deck+Template+-+Fusion+Event.pdf)並觀看隨選錄製。

如需可下載的CSV檔案範例和Fusion CSV套件組合，請檢視[Experience League社群貼文](https://experienceleaguecommunities.adobe.com/t5/workfront-discussions/event-follow-up-fast-track-efficiency-data-imports-with/td-p/769522)！

每月都會新增事件，因此請務必檢視[Experience League事件頁面](https://experienceleague.adobe.com/zh-hant/events)以瞭解最新工作階段。

