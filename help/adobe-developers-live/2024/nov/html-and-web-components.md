...
description: 透過Adobe的Raymond Camden， Sr.開發人員宣傳專員，瞭解Web元件基本知識，包括自訂元素、陰影DOM和HTML範本，以及內嵌PDF和建立可排序表格以增強應用程式等實際範例。
...
加入Adobe資深開發人員宣傳專員Raymond Camden，瞭解Web元件基本知識，包括自訂元素、陰影DOM和HTML範本。 探索真實世界的範例，例如內嵌PDF和建立可排序的表格，以透過可重複使用的現代解決方案增強您的應用程式。
...
* **Web元件簡介** Web元件可讓開發人員建立自訂HTML元素，並使用JavaScript定義其外觀和互動性。
...
* **HTML範本** HTML範本允許複製並附加至DOM的HTML和CSS定義。 不過，簡報者偏好使用插槽，而非範本，以提升發佈與彈性。
...
* 如果HTML無法載入，**漸進式增強功能** Web元件可以增強現有JavaScript而不中斷功能。
* **後續步驟和資源**&#x200B;簡報建議探索表單參與率、宣告式陰影DOM、自訂HTML屬性，以及伺服器端轉譯。 資源包括MDN、webcomponents.org以及Ben Farrell的《Web Components in Action》一書。
